        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <div className="mb-4 sm:mb-6">
            ...
            <div className="text-xs text-gray-500">
              Step {currentStep + 1} of {generationSteps.length} • {Math.round(overallProgress)}% complete
            </div>
          </div>
        </div>
...
const StepIndicator = ({ 
  ...
  <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 lg:mb-4">
    ...
    <h2 className={`text-base sm:text-lg lg:text-lg font-medium transition-colors duration-300 ${
...
export const GenerateVideoUI = () => {
  ...
  const videoType = useMemo(() => videoTypeConfig.map(config => ({
    name: config.name,
    child: config.child,
    icon: config.icon,
    desc: config.desc
  ...
  const languages = useMemo(() => languageConfig.map(config => ({
    name: config.name,
    flag: config.flag,
    desc: config.desc
  })), [languageConfig]);
  ...
  const progressData = useMemo(() => {
    const steps = {
      ...
      disease: selectedVideoType === "Educational Videos" || selectedDisease !== null
    };
  ...
  const handleLanguageSelect = useCallback((lang: string) => {
    ...
    if (selectedVideoType) {
      const typeConfig = videoTypeConfig.find(vt => vt.name === selectedVideoType);
      if (typeConfig && typeConfig.available[lang as keyof typeof typeConfig.available]) {
  ...
  const handleVideoTypeSelect = useCallback((name: string, child: string[]) => {
    ...
    if (selectedLanguage) {
      ...
      if (typeConfig && typeConfig.available[selectedLanguage as keyof typeof typeConfig.available]) {
        const availableForLang = typeConfig.available[selectedLanguage as keyof typeof typeConfig.available];
        setSelectDisease(availableForLang || []);
      } else {
  ...
  return (
    <>
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/90 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 mt-4">Finalizing your video...</p>
          </div>
        </div>
      )}
      ...
      <div className={`min-h-screen bg-gray-50 pt-20 ${show ? "hidden" : "block"}`}>
        ...
        >
          <DialogTitle className="text-center font-medium text-lg text-gray-900">
          ...
          <DialogContent>
            ...
            <div className={`py-6 ${request ? "hidden" : "block"}`}>
              <div className="text-center space-y-4">
                <div className="w-12 h-12 bg-[#2463e9] mx-auto flex items-center justify-center">
                  <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <h3 className="text-xl font-medium text-gray-900">Your Video, Your Way</h3>
                <p className="text-gray-600 max-w-md mx-auto">
                  Create a completely custom video experience tailored to your specific needs.
                </p>
                <button
                  onClick={() => setRequest(true)}
                  className="mt-6 bg-[#2463e9] hover:bg-[#1952d4] text-white px-6 py-3 font-medium transition-colors"
                >
        ...
        <div className="max-w-4xl mx-auto p-2 sm:p-3 md:p-4 lg:p-5 space-y-3 sm:space-y-4 md:space-y-5 lg:space-y-6">
          ...
          <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
            ...
            <div className="hidden sm:flex items-center gap-1.5 sm:gap-2 bg-blue-50 px-2 sm:px-3 lg:px-3 py-1 sm:py-1.5 lg:py-1.5 rounded-xl">
              ...
              <span className="text-blue-700 text-xs sm:text-sm font-medium">AI Powered</span>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-white to-blue-50 border border-blue-100 rounded-xl p-2 sm:p-3 md:p-4 lg:p-4 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 sm:mb-3 lg:mb-3 gap-2 sm:gap-3 lg:gap-0">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 text-xs sm:text-sm lg:text-base">Setup Progress</h3>
                  <p className="text-xs text-gray-600 leading-tight">Complete all steps to generate</p>
                </div>
              </div>
              <div className="text-right">
                <div className="text-lg sm:text-xl lg:text-2xl font-bold text-blue-600">{progressData.percentage}%</div>
                <div className="text-xs text-gray-500">{progressData.completed}/4 steps</div>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <div className="grid grid-cols-2 sm:flex sm:justify-between text-xs font-medium gap-1">
                <span className={`${progressData.steps.avatar ? 'text-blue-600' : 'text-gray-400'} transition-colors text-center sm:text-left`}>
                  Avatar {progressData.steps.avatar && '✓'}
                </span>
                <span className={`${progressData.steps.language ? 'text-blue-600' : 'text-gray-400'} transition-colors text-center sm:text-left`}>
                  Language {progressData.steps.language && '✓'}
                </span>
                <span className={`${progressData.steps.videoType ? 'text-blue-600' : 'text-gray-400'} transition-colors text-center sm:text-left`}>
                  Video Type {progressData.steps.videoType && '✓'}
                </span>
                <span className={`${progressData.steps.disease ? 'text-blue-600' : 'text-gray-400'} transition-colors text-center sm:text-left`}>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-2 sm:p-3 md:p-4 lg:p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4">
              {avatarArray.map((item, index) => (
                ...
                >
                  <div className="bg-gradient-to-b from-gray-50 to-white p-2 sm:p-3 lg:p-4 hover:from-blue-50 hover:to-white transition-all duration-200 rounded-lg border border-gray-200">
                    <div className="aspect-square relative bg-gray-100 mb-2 sm:mb-3 lg:mb-3 overflow-hidden rounded-lg shadow-sm">
                      {index === 3 ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-purple-100 to-indigo-100">
                          <div className="text-center">
                            <svg className="w-8 h-8 sm:w-12 sm:h-12 lg:w-16 lg:h-16 text-purple-600 mx-auto mb-1 sm:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                            </svg>
                            <div className="text-xs font-medium text-purple-700">Create Own</div>
                          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-2 sm:p-3 md:p-4 lg:p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 lg:gap-4">
              {languages.map((lang) => {
                ...
                const isAvailable = selectedVideoType ? 
                  videoTypeConfig.find(vt => vt.name === selectedVideoType)?.available[lang.name as keyof typeof videoTypeConfig[0]['available']] :
                  true;
                ...
                return (
                  ...
                  >
                    <div className={`bg-gradient-to-r from-gray-50 to-white p-2 sm:p-3 lg:p-4 hover:from-blue-50 hover:to-white transition-all duration-200 rounded-lg border border-gray-200 ${!isAvailable ? 'opacity-60' : ''}`}>
                      <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
                        <div className="text-lg sm:text-2xl lg:text-3xl">{lang.flag}</div>
                        <div className="flex-1">
                          <h3 className="font-semibold text-gray-900 text-xs sm:text-sm lg:text-base mb-0.5 sm:mb-1">{lang.name}</h3>
                        ...
                        {!isAvailable && selectedVideoType && (
                          <div className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 bg-orange-100 rounded-full flex items-center justify-center">
                            <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-4 lg:h-4 text-orange-500" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="mt-4 text-center">
              <button
                ...
                disabled={loading || showPersonalizedLoader}
                className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors text-xs sm:text-sm text-gray-600 disabled:opacity-50"
              >
                <Image src="/custom.png" alt="custom" width={14} height={14} className="sm:w-4 sm:h-4" />
                ...
                >
                  <div className="bg-gradient-to-b from-gray-50 to-white p-2 sm:p-3 lg:p-4 hover:from-blue-50 hover:to-white transition-all duration-200 h-full rounded-lg border border-gray-200">
                    <div className="flex flex-col items-center text-center space-y-2 sm:space-y-3 lg:space-y-3 h-full">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900 text-xs sm:text-sm lg:text-base mb-0.5 sm:mb-1 lg:mb-1">{name}</h3>
          ...
          {diseaseList.length > 0 && (
            <div className="bg-white/70 backdrop-blur-sm rounded-xl p-2 sm:p-3 md:p-4 lg:p-5">
              <StepIndicator 
                ...
                isCompleted={selectedDisease !== null || selectedVideoType === "Educational Videos"}
                isActive={selectedVideoType !== null && (selectedDisease === null && selectedVideoType !== "Educational Videos")}
              />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 lg:gap-3">
                {diseaseList.map((disease) => (
                  <SelectionCard
                    ...
                    disabled={loading || showPersonalizedLoader}
                    onClick={() => setSelectedDisease(prev => prev === disease ? null : disease)}
                  >
                    <div className="bg-white p-2 sm:p-3 lg:p-3 hover:bg-gray-50 transition-colors rounded-lg">
                      <div className="flex items-center justify-between">
                        <h3 className="font-medium text-gray-900 text-xs sm:text-sm">{disease}</h3>
            ...
            {isFormValid ? (
              <div className="text-center w-full sm:w-auto">
                <button
                  ...
                  disabled={loading || showPersonalizedLoader}
                  className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 sm:px-8 lg:px-10 py-3 sm:py-4 text-base sm:text-lg font-semibold rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 sm:gap-4 shadow-lg hover:shadow-xl transform hover:scale-105 disabled:transform-none"
                >
                  {showPersonalizedLoader ? (
                    <>
                      <CircularProgress size={20} style={{ color: "white" }} className="sm:w-6 sm:h-6" />
                      <span className="text-sm sm:text-base">Creating Your Video...</span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-sm sm:text-base">Generate Video</span>
                      <Image src="/right_use.png" alt="arrow" width={16} height={16} className="sm:w-5 sm:h-5" />
                    </>
                  )}
                </button>
              ...
              <Tooltip title="Please complete all required selections to generate your video">
                <div className="text-center w-full sm:w-auto">
                  ...
                  >
                    <svg className="w-5 h-5 sm:w-6 sm:h-6 opacity-50" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-2 sm:p-3 md:p-4 lg:p-4 border border-blue-100">
            <div className="flex items-start gap-2 sm:gap-3 lg:gap-3">
              <div className="w-6 h-6 sm:w-8 sm:h-8 lg:w-9 lg:h-9 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                <svg className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
